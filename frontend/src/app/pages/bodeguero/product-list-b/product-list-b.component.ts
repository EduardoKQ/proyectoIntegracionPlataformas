...
import { catchError, map, tap, takeUntil, switchMap, finalize } from 'rxjs/operators';
...
@Component({
  ...
  templateUrl: './product-list-b.component.html',
  styleUrls: ['./product-list-b.component.scss']
})
export class ProductListComponentB implements OnInit, OnDestroy {
  ...
  private bodegueroBranchCode: string | null = null;
  public bodegueroBranchName: string | null = null;
      ...
      return this.http.get<any>(`${this.userApiUrl}/me`).pipe(
        ...
        map(workerDetails => {
          if (workerDetails && workerDetails.branch && workerDetails.branch.branch_code) {
            ...
            this.errorMessage = 'Bodeguero sin sucursal asignada o datos incompletos.';
            this.bodegueroBranchName = null;
            throw new Error(this.errorMessage);
          }
        }),
        switchMap(() => this.fetchProductsAndInventory()),
        catchError(err => {
          this.errorMessage = this.errorMessage || err.message || 'Error al obtener detalles del bodeguero.';
          this.isLoading = false;
          this.productsWithStock$ = of([]);
          this.bodegueroBranchName = null;
          return EMPTY;
  ...
  private fetchProductsAndInventory(): Observable<any[]> {
    ...
    this.productsWithStock$ = forkJoin({
      ...
      map(data => {
        ...
        let processedProducts = apiProducts.map(product => {
          ...
          if (this.currentUserRole === 'bodeguero' && this.bodegueroBranchCode) {
            stockForThisProduct = inventoryItems
              .filter(item => item && item.product_code === product.codigo_producto && item.branch_code === this.bodegueroBranchCode)
        ...
        if (this.currentUserRole === 'bodeguero' && this.bodegueroBranchCode) {
          const branchCode = this.bodegueroBranchCode;
